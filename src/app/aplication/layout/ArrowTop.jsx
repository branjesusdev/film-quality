...
function ArrowTop () {
  ...
  const { scrollTop: isFixed } = useIsScrollTop({ elementById: fixedElement, positonIdentity: 1600 })

  return (
    <div
      id={fixedElement} className={`${isFixed ? 'block' : 'hidden'} transition-all fixed bottom-16 right-16 border-custom p-3 rounded-full bg-black-light shadow-md
